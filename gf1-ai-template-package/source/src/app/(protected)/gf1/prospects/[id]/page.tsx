...
import { fetchAssignableUsers, fetchProspectProfile } from '@/lib/gf1/queries';
...
export default async function ProspectProfilePage({ params: paramPromise, searchParams }: ProspectProfilePageProps) {
  ...
  const [profile, assignees] = await Promise.all([
    fetchProspectProfile(params.id, supabase),
    fetchAssignableUsers(supabase),
  ]);

  if (!profile || !profile.prospect) {
    notFound();
  }
  ...
  const basePath = `/gf1/prospects/${params.id}`;
  ...
  const assignedName = lookupUserName(profile.prospect.assigned_rep_id, assignees);
    ...
    <div className="space-y-6">
      <section className="rounded-2xl p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase text-neutral-400">Prospect · {profile.prospect.stage}</p>
...
function renderTabContent(
  tab: string,
  profile: NonNullable<Awaited<ReturnType<typeof fetchProspectProfile>>>,
  ...
  switch (tab) {
    ...
    case 'services':
      return <ServicesTab profile={profile} role={role} canEdit={canEdit} />;
    case 'agreements':
      ...
      <div className="space-y-4">
        <article className="rounded-xl p-4">
          <dl className="grid gap-4 sm:grid-cols-3">
            <div>
              <dt className="text-xs uppercase text-neutral-400">Probability</dt>
              <dd className="text-lg font-semibold text-neutral-800">
                {profile.prospect?.probability ? `${profile.prospect.probability}%` : '—'}
              </dd>
            </div>
            <div>
              <dt className="text-xs uppercase text-neutral-400">Target close</dt>
              <dd className="text-lg font-semibold text-neutral-800">
                {profile.prospect?.target_close_date ? new Date(profile.prospect.target_close_date).toLocaleDateString() : '—'}
              </dd>
            </div>
            <div>
        ...
        <article className="rounded-xl p-4">
          ...
          <ul className="text-sm text-neutral-600">
            ...
            <li>Client: {profile.client ? profile.client.status : 'Not yet converted'}</li>
          </ul>
        </article>
      </div>
      <div className="space-y-4">
        ...
        <ReassignOwnerForm
          entityType="prospect"
          entityId={profile.prospect?.id ?? ''}
          currentAssigneeId={profile.prospect?.assigned_rep_id ?? null}
          users={assignees}
          viewerRole={role}
        />
      </div>
