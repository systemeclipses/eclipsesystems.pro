...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const result = await transitionPtoRequest(params.id, "CANCEL", { note: typeof body.note === "string" ? body.note : null }, { organizationId, actorMembershipId });
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to cancel request." }, { status: 400 });
  }
}
