...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const result = await transitionPtoRequest(params.id, "SEND_BACK", { note: typeof body.note === "string" ? body.note : "" }, { organizationId, actorMembershipId });
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to send back request." }, { status: 400 });
  }
}
