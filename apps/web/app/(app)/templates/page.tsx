        ...
        {packageDefinitions.map((pkg) => {
          ...
          return (
            <Link
              key={pkg.name}
              href={pkg.href}
              className="group flex min-h-[480px] flex-col justify-between rounded-md border border-border bg-white/70 p-5 shadow-sm shadow-primary/5 transition hover:-translate-y-0.5 hover:border-primary hover:bg-white"
            >
              <div>
                <div className="flex items-start justify-between gap-3">
                  <span className="grid h-12 w-12 place-items-center rounded-sm bg-secondary text-primary">
                    <Icon className="h-6 w-6" />
                  </span>
                  <ArrowRight className="h-5 w-5 text-muted-foreground transition group-hover:translate-x-0.5 group-hover:text-primary" />
                </div>
                <h2 className="mt-6 text-3xl font-semibold leading-none text-ink">{pkg.name}</h2>
                <p className="mt-4 text-base font-semibold leading-6 text-primary">{pkg.tagline}</p>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">For: {pkg.audience}.</p>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {pkg.features.slice(0, 4).map((feature) => (
                  <span key={feature} className="rounded-sm bg-cream px-2 py-1 text-xs font-semibold text-primary">{feature}</span>
                ))}
              </div>
            </Link>
          );
