...
export async function generateMetadata({ params }: { params: { plan: string } }): Promise<Metadata> {
  ...
  if (!plan) return {};
  return { title: `${plan.name} Plan`, description: plan.summary, alternates: { canonical: `/plans/${plan.slug}` } };
}
...
export default async function PlanPage({ params }: { params: { plan: string } }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-12">
      ...
      <h1 className="text-3xl font-semibold">Eclipse Timekeeping {plan.name} Plan</h1>
      ...
      <FAQ items={[
        { question: `How much does the ${plan.name} plan cost?`, answer: `${plan.name} costs $${plan.priceMonthly} per seat per month, or $${plan.annualEffectiveMonthly} effective monthly per seat when billed annually.` },
        { question: `Is the ${plan.name} plan available as a trial?`, answer: "Yes. Every paid Eclipse Timekeeping plan includes a 14-day card-required trial." },
        { question: `Can teams change from ${plan.name} later?`, answer: "Yes. Teams can change plans through Stripe billing workflows as their needs expand." }
      ]} />
    </main>
  );
}
