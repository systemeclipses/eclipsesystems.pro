...
import { demoConfig, pipelineStages, type Activity, type Deal, type DealStage, type Product } from "@/lib/crm-demo-data";
import { dealValue, getCompany, getContact, getOwner, isLastMonth, isThisMonth, stageProbability, useCrmStore, type ViewKey } from "@/lib/crm-demo-store";

const stageColors: Record<DealStage, string> = {
  "New Lead": "#a5b07a",
  Qualified: "#7f9a76",
  "Proposal Sent": "#5f7f67",
  Negotiation: "#3f634d",
  "Closed Won": "#2f4135",
  "Closed Lost": "#9b6a5f"
};
...
export function TemplateCrmDemo({ initialView = "dashboard" }: { initialView?: ViewKey }) {
  const store = useCrmStore();
  const setInitialView = useCrmStore((state) => state.setView);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="grid min-h-screen lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="hidden border-r border-white/10 bg-[#2f4135] p-4 text-white lg:block">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-sm bg-cream text-lg font-bold text-primary">E</span>
            <div>
              ...
              return (
                <button
                  ...
                  className={cx(
                    ...
                    active ? "bg-[#f4eadb] text-[#1f3428] shadow-sm" : "text-white/72 hover:bg-white/10 hover:text-white"
                  )}
                >
          ...
          <header className="sticky top-0 z-30 border-b border-border bg-panel/95 px-4 py-3 backdrop-blur lg:px-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <h2 className="text-2xl font-semibold">{timeGreeting}, {store.users[0]?.name ?? "there"}</h2>
              </div>
...
function Dashboard({ deals }: { deals: Deal[] }) {
  ...
  const industryData = state.companies
    .map((company) => {
      const industryDeals = deals.filter((deal) => deal.companyId === company.id && deal.stage !== "Closed Lost");
      const industryContacts = state.contacts.filter((contact) => contact.companyId === company.id);
      return {
        industry: company.industry,
        contacts: industryContacts.length,
        deals: industryDeals.length,
        value: industryDeals.reduce((sum, deal) => sum + dealValue(deal), 0),
        open: industryDeals.filter((deal) => !deal.stage.startsWith("Closed")).length,
        won: industryDeals.filter((deal) => deal.stage === "Closed Won").length
      };
    })
    .filter((item) => item.value > 0)
    .reduce<Array<{ industry: string; contacts: number; deals: number; value: number; open: number; won: number }>>((acc, item) => {
      const existing = acc.find((entry) => entry.industry === item.industry);
      if (existing) {
        existing.contacts += item.contacts;
        existing.deals += item.deals;
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={repData} layout="vertical" margin={{ left: 12, right: 18 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(255,255,255,0.35)" />
                  <XAxis type="number" tickFormatter={(value) => `$${Number(value) / 1000}k`} tick={{ fill: "#ffffff", fontSize: 12 }} axisLine={{ stroke: "rgba(255,255,255,0.45)" }} tickLine={{ stroke: "rgba(255,255,255,0.45)" }} />
                  <YAxis dataKey="name" type="category" width={70} tick={{ fill: "#ffffff", fontSize: 13 }} axisLine={{ stroke: "rgba(255,255,255,0.45)" }} tickLine={false} />
                  <Tooltip formatter={(value) => formatMoney(Number(value))} />
                  <Bar dataKey="value" fill="hsl(var(--primary))" radius={[0, 5, 5, 0]} isAnimationActive />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Panel>
          <Panel title="Industry traction" icon={Building2}>
            <IndustryTraction data={industryData} />
          </Panel>
        </div>
      </section>
...
function StageFunnel({ data }: { data: Array<{ name: DealStage; count: number; value: number; fill: string }> }) {
  ...
  return (
    <div className="grid gap-3">
      {data.map((item, index) => {
        ...
        const valueWidth = Math.max(6, Math.round((item.value / maxValue) * 100));
        return (
          <article key={item.name} className="crm-demo-stage-row rounded-md border border-border bg-muted/45 p-3" style={{ animationDelay: `${index * 80}ms` }}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-sm text-xs font-semibold text-white" style={{ backgroundColor: item.fill }}>
                  {index + 1}
                </span>
              ...
              <p className="shrink-0 text-sm font-semibold">{formatMoney(item.value)}</p>
            </div>
            <div className="mt-3 grid gap-2">
              <div className="h-4 overflow-hidden rounded-sm bg-white">
                <div className="crm-demo-bar h-full origin-left rounded-sm" style={{ width: `${width}%`, backgroundColor: item.fill, animationDelay: `${160 + index * 80}ms` }} />
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-white">
                <div className="crm-demo-bar h-full origin-left rounded-full bg-primary/70" style={{ width: `${valueWidth}%`, animationDelay: `${260 + index * 80}ms` }} />
              </div>
            </div>
          </article>
        );
...
function IndustryTraction({ data }: { data: Array<{ industry: string; contacts: number; deals: number; value: number; open: number; won: number }> }) {
  const maxValue = Math.max(...data.map((item) => item.value), 1);
  ...
  return (
    <div className="grid gap-3">
      {data.map((item, index) => {
        ...
        return (
          <article key={item.industry} className="crm-demo-stage-row rounded-md border border-border bg-muted/45 p-3" style={{ animationDelay: `${index * 70}ms` }}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="font-semibold">{item.industry}</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {item.open} open · {item.won} won · {item.contacts} contacts
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-[#f4eadb]">{formatMoney(item.value)}</p>
                <p className="mt-1 text-xs text-white/72">{formatMoney(Math.round(item.value / Math.max(item.deals, 1)))} avg</p>
              </div>
            </div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-white">
              <div className="crm-demo-bar h-full origin-left rounded-full bg-secondary" style={{ width: `${width}%`, animationDelay: `${180 + index * 70}ms` }} />
            </div>
          </article>
        );
      })}
    ...
    <article ref={setNodeRef} style={style} className={cx("rounded-md border border-border bg-panel p-3 shadow-soft", isDragging && "z-20 opacity-70")}>
      <button type="button" onClick={onOpen} className="block w-full text-left">
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          <span className="font-semibold text-primary dark:text-secondary">{formatMoney(dealValue(deal))}</span>
      ...
      <Panel title="Contacts" icon={UsersRound}>
        <div className="grid gap-2">
          {state.contacts.map((item) => (
            <button key={item.id} onClick={() => state.setSelectedContact(item.id)} className={cx("rounded-md border p-3 text-left", item.id === contact.id ? "border-primary bg-secondary/25" : "border-border hover:bg-muted")}>
      ...
      <Panel title="Companies" icon={Building2}>
        <div className="grid gap-2">
          {state.companies.map((item) => (
            <button key={item.id} onClick={() => state.setSelectedCompany(item.id)} className={cx("rounded-md border p-3 text-left", item.id === company.id ? "border-primary bg-secondary/25" : "border-border hover:bg-muted")}>
              <p className="font-semibold">{item.name}</p>
        ...
        return (
          <article key={activity.id} className="flex items-start gap-3 rounded-md border border-border p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {!compact && activity.dueDate ? <p className="mt-1 text-sm font-semibold text-primary dark:text-secondary">Due {formatDate(activity.dueDate)}</p> : null}
            </div>
...
function Modal({ title, children, onClose, wide = false }: { title: string; children: React.ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-transparent p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={title}>
      <section className={cx("max-h-[90vh] w-full overflow-y-auto rounded-md border border-border bg-[#f4eadb] p-5 text-[#1f3428] shadow-2xl shadow-black/30 dark:border-white/10 dark:bg-[#15231a] dark:text-[#f4eadb] [&_input]:bg-white [&_input]:text-[#1f3428] [&_select]:bg-white [&_select]:text-[#1f3428] [&_textarea]:bg-white [&_textarea]:text-[#1f3428] dark:[&_input]:border-white/10 dark:[&_input]:bg-[#0f1a14] dark:[&_input]:text-[#f4eadb] dark:[&_select]:border-white/10 dark:[&_select]:bg-[#0f1a14] dark:[&_select]:text-[#f4eadb] dark:[&_textarea]:border-white/10 dark:[&_textarea]:bg-[#0f1a14] dark:[&_textarea]:text-[#f4eadb]", wide ? "max-w-5xl" : "max-w-xl")}>
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2 className="text-xl font-semibold">{title}</h2>
